...
/**
 * Tests for create:module:sub (CreateModuleSub).
 */
final class CreateModuleSubTest extends CommandTestCase
{
    ...
    {
        $this->assertArrayHasKey('create:module:sub', Artisan::all());
    }

    public function test_command_exposes_only_real_options(): void
    {
        $command = Artisan::all()['create:module:sub'];
        $this->assertTrue($command->getDefinition()->hasOption('fillable'));
        $this->assertTrue($command->getDefinition()->hasOption('db-only'));
    ...
    {
        try {
            $this->artisan('create:module:sub', ['module' => $this->moduleName])->assertFailed();
